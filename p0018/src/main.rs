...
fn main() {
	println!("{}", path_sum(0, 0, 0));
}

fn path_sum(level: usize, index: usize, mut sum: i32) -> i32 {
	sum += TRIANGLE[level][index];

	if level < TRIANGLE.len() - 1 {
		let left = path_sum(level + 1, index, sum);
		let right = path_sum(level + 1, index + 1, sum);
		if right > left {
			return right;
		} else {
			return left;
